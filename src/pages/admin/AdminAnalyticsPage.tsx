...
export function AdminAnalyticsPage() {
  ...
  const [difficultyStats, setDifficultyStats] = useState<DifficultyBreakdown[]>([]);
  ...
  const maxUsersStarted = Math.max(...chapterStats.map((c) => c.users_started), 1);
  const totalButterflyRecords = butterflyDist.reduce((sum, b) => sum + b.count, 0);
    ...
    <AdminLayout>
      <div className="space-y-8">
        <div>
          ...
          <p className="text-sm text-gray-400">Learning progress and engagement metrics</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            ...
            return (
              <div key={stat.label} className={`bg-gray-900 border ${c.border} rounded-xl p-5`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <div className="bg-gray-900 border border-gray-800 rounded-xl">
            <div className="p-6 border-b border-gray-800">
              <h2 className="text-sm font-semibold text-white">Butterfly Stage Distribution</h2>
              ...
              {butterflyDist.map((item) => {
                ...
                return (
                  <div key={item.stage}>
                    ...
                    <BarChart value={item.count} max={totalButterflyRecords} color={config.bg} />
                  </div>
                );
              })}
            </div>
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl">
            <div className="p-6 border-b border-gray-800">
              <h2 className="text-sm font-semibold text-white">Mastery by Difficulty</h2>
              <p className="text-xs text-gray-400 mt-1">Terms mastered per difficulty level</p>
            </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-xl">
          <div className="p-6 border-b border-gray-800">
            <h2 className="text-sm font-semibold text-white">Chapter Progress Overview</h2>
            <p className="text-xs text-gray-400 mt-1">User engagement per chapter</p>
          </div>
              ...
              <tbody className="divide-y divide-gray-800/50">
                {chapterStats.map((ch) => (
                  <tr key={ch.chapter_id} className="hover:bg-gray-800/20">
                    ...
                    <td className="px-6 py-3 text-sm text-gray-400">{ch.term_count}</td>
                    <td className="px-6 py-3 text-sm text-gray-300">{ch.users_started}</td>
                    <td className="px-6 py-3 text-sm text-gray-300">{ch.users_completed}</td>
                    <td className="px-6 py-3 text-sm text-gray-300">{ch.avg_terms_mastered}</td>
                    <td className="px-6 py-3">
                      <BarChart
